...
import ethereumIdentityStoreContract from '../build/contracts/EthIdentityStore.json'
...
class App extends Component {
  ...
  instantiateContract() {
     ...
     */
     ...
     const ethereumIdentityStore = contract(ethereumIdentityStoreContract)
     ethereumIdentityStore.setProvider(this.state.web3.currentProvider)

    // Declaring this for later so we can chain functions on ethereumIdentityStore.
    var ethereumIdentityStoreInstance

    // Get accounts.
    this.state.web3.eth.getAccounts((error, accounts) => {
      ethereumIdentityStore.deployed().then((instance) => {
        ethereumIdentityStoreInstance = instance
        // console.log(ethereumIdentityStoreInstance)
        // Stores a given value, 5 by default.
        // return ethereumIdentityStoreInstance.set(10, {from: accounts[0]})
      // }).then((result) => {
        // Get the value from the contract to prove it worked.
        this.setState({contract: ethereumIdentityStoreInstance, account: accounts[0]} )
        // ethereumIdentityStoreInstance.enroll("Wael", "Yousfi","10/17/1992","Tunisian","Tunisia",{ from: accounts[0] })
        return ethereumIdentityStoreInstance.isSet.call(accounts[0])
      }).then((result) => {
        // Update state with the result.
        return ethereumIdentityStoreInstance.myIdentityStore.call(accounts[0])
        .then((result) => {
  ...
  update(event) {
    ...
    if (this.state.FN != null) {
      ...
      .then(result => {
        return contract.myIdentityStore.call({ from: account })
      }).then(result => {
  ...
  render() {
    return (
      ...
      <a href="#" className="pure-menu-heading pure-menu-link">Ethereum IdentityStore</a>
      </nav>
